...
import {installableConfig,inviteUrl,redactedError} from './installable-config.mjs';
import {fetchRecords,journals} from './journals.mjs';
import {attachmentFor} from './attachments.mjs';
import {guildState,configureGuild,scanGuild,resolveGuildDelivery} from './service-core.mjs';
import {withLock} from './state.mjs';
import {guardedListener} from './event-handler.mjs';

loadEnv();const cfg=installableConfig();
if(!cfg.token){console.error('Run npm run setup first to connect your Discord bot token.');process.exit(1);}
...
async function ensureChannel(guild){
  ...
  if(state?.channelId){
    ...
    if(!usable(channel,guild))throw new Error(`${guild.name}: configured channel unavailable. Select another with /journal channel.`);
    return channel;
  }
  ...
  if(!channel)throw new Error(`${guild.name}: no writable text channel. Grant channel permissions or use /journal channel.`);
  await configureGuild(cfg,guild.id,{channelId:channel.id});return channel;
}
async function send({channelId,payload,attachment,messageId,nonce}){
  const channel=await client.channels.fetch(channelId);
  const options={content:payload.content,embeds:payload.embeds,allowedMentions:{parse:[]},...(attachment?{files:[{attachment:attachment.bytes,name:attachment.name,description:payload.attachments[0].description}],attachments:[]}:{}),...(!messageId?{nonce,enforceNonce:true}:{})};
  try{return (messageId?await channel.messages.edit(messageId,options):await channel.send(options)).id;}
  catch(e){e.ambiguous=!(e.status>=400&&e.status<500);throw e;}
}
...
const command=new SlashCommandBuilder().setName('journal').setDescription('Manage Canadian chemical engineering journal posts').setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild).setDMPermission(false)
  ...
  .addSubcommand(c=>c.setName('resolve').setDescription('Resolve uncertain delivery after checking channel history').addStringOption(o=>o.setName('issue').setDescription('For example cjce:104:9').setRequired(true)).addStringOption(o=>o.setName('result').setDescription('Confirmed message ID, or retry only if the post was not delivered').setRequired(true)));

client.on(Events.InteractionCreate,guardedListener(async interaction=>{
  if(!interaction.isChatInputCommand()||interaction.commandName!=='journal'||!interaction.guild)return;
  ...
  try{
    const text=await enqueue(async()=>{
      ...
      if(sub==='channel'){
        ...
        if(!usable(channel,guild))throw new Error('The bot needs View Channel, Send Messages, Embed Links and Read Message History there.');
      ...
      if(sub==='resolve'){
        ...
        if(result!=='retry'){
          const target=await guildState(cfg,guild.id);const channel=await client.channels.fetch(target.channelId);const message=await channel.messages.fetch(result);
      ...
      const target=await guildState(cfg,guild.id);if(!target)return 'Setup is pending. Use /journal channel to select a destination.';
      const pending=Object.values(target.issues).filter(i=>i.status==='pending').map(i=>i.issueKey);
...
async function run(){
  await withLock(path.join(cfg.stateDir,'runtime'),async()=>{
    ...
    if(identityResponse.status===401)throw new Error('Discord rejected the saved bot token. Run npm run setup to save your replacement token; keep data/installable to preserve delivery history.');
    ...
    await new Promise((resolve,reject)=>{
      client.once(Events.ClientReady,async()=>{
        try{
          ...
          console.log(`Online: ${client.user.username}. Invite: ${inviteUrl(client.application.id)}`);
