...
export async function scanGuild(cfg,guildId,records,{now=new Date(),force=false,refreshExisting=false,send,attachmentFor=async()=>null,log=console.log}={}) {
  return withLock(cfg.stateDir,async file=>{
    ...
    if(Object.entries(target.issues).some(([key,value])=>key.startsWith(channelId+':')&&value.status==='pending'))throw new Error('Delivery needs review. Use /journal status and /journal resolve before retrying.');
    ...
    for(const issue of issues){
      ...
      const pending={status:'pending',issueKey:issue.key,channelId,messageId:prior?.messageId,contentHash:hash,pdfHash,pdfName:attachment?.name||prior?.pdfName||null,startedAt:now.toISOString()};
      target.issues[key]=pending;await saveState(file,state);
      let id;
      try{
        id=await send({guildId,channelId,issue,payload,attachment,messageId:prior?.messageId,nonce:BigInt('0x'+createHash('sha256').update(`${guildId}:${key}`).digest('hex').slice(0,16)).toString()});
        if(!/^\d+$/.test(id||''))throw new Error('Discord did not confirm a message ID');
      }catch(e){
        if(e.ambiguous===false){if(prior)target.issues[key]=prior;else delete target.issues[key];await saveState(file,state);}
        throw e;
      }
...
export async function resolveGuildDelivery(cfg,guildId,issueKey,messageId){
  ...
  return withLock(cfg.stateDir,async file=>{
    ...
    if(messageId==='retry'){if(item.messageId)target.issues[key]={...item,status:'sent',contentHash:null};else delete target.issues[key];}
    else target.issues[key]={...item,status:'sent',messageId};
